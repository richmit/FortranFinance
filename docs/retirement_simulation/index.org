...
:END:
  ...
  - For my situation in particular, careful tax panning is a larger factor in a successful retirement than my portfolio mix!
...
:END:
  ...
  - IRA to Roth IRA conversions (*in work*)
    - I want to trigger conversions early in the retirement phase when we have low taxes from using the brokerage.
    - The idea will be to set a target tax rate, and convert enough each year to meet that target.
    - Tax impact of regular IRA to Roth IRA conversions 
  - Correlated banded and inflation MC
  - Black–Scholes MC
...
This graph also deals with expenses, but shows how the expenses are paid.
...
simulation runs.  The yellow band, which includes the green band, contains 80% of all simulation runs.  You can change the bands in the ~retire.R~ script.

  [[file:pics/savings_crg_20_mc_full.png][file:pics/savings_crg_20_mc_800x.png]]

This graph shows 2000 savings trajectories, all the failure trajectories (when we went broke), and an envelope for the highest balance for a trajectories that
...
This graph is similar to the probability band graph, but is a more traditional presentation for people accustom to box plots.
