...
You can analyze the data however you wish, but I have included an R script if that's your thing.  It will consume the output of the simulation, and produce a
...
*** When retire.out contains a multiple simulations (monte carlo), then eight visualizations are produced:
:PROPERTIES:
